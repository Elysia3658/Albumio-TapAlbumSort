...
class AlbumFolderPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Album> {
        ...
        // 计算结束索引，确保不超过列表边界
        val toIndex = minOf(fromIndex + pageSize, albumList.size)

        // 从完整列表中截取当前页的数据子集
        val sublist = if (fromIndex < albumList.size) {
            albumList.subList(fromIndex, toIndex)  // 安全截取子列表
        } else {
        ...
        return LoadResult.Page(
            ...
            nextKey = if (toIndex < albumList.size) page + 1 else null  // 下一页键：如果还有数据则为下一页编号
        )
    }
